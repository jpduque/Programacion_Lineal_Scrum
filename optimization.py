...
class opt:
    def model(seniorDevCost, techDevCost, juniorDevCost, internDevCost, seniorQACost, techQACost, juniorQACost,
              internQACost, seniorDevDeliver, techDevDeliver, juniorDevDeliver, internDevDeliver, seniorQADeliver,
              techQADeliver, juniorQADeliver, internQADeliver, sprintPoints, projectPoints, projectBudget):
        global x1
        global x2
        global x3
        global x4
        ...
        c14 = (x8 >= 0)
        lp1 = op(
            int(seniorDevCost) * x1 + int(techDevCost) * x2 + int(juniorDevCost) * x3 + int(internDevCost) * x4 + int(
